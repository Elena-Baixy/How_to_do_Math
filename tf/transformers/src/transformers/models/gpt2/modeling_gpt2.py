...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, key_list = [[]],
        value_list = [[]], num_example_attn = 0): #FIXME
        super().__init__()
        ...
        self.num_example_attn = num_example_attn
    ...
    def forward(
        ...
        layer_past: Optional[Tuple[torch.Tensor]] = None, #FIXME
        ...
        query = self._split_heads(query, self.num_heads, self.head_dim) # 12,11,64 by spliting hidden into num_heads
        ...
        if layer_past is not None: #kvcache: 在一个token past过以后，presents里的kv cache 会存入layer_past
        ...
        #breakpoint()
        return self.key_list, self.value_list, self.num_example_attn, outputs  # a, present, (attentions)


class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config, example_list = [[]],num_example = 0):
        super().__init__()
        ...
        self.example_list = example_list # each column is a list of one example for 12 layers, row is each example
...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        #breakpoint() # hidden_states [1,11,768]
        hidden_states = self.ln_1(hidden_states) #FIXME
        ...
        #FIXME: attention
        #breakpoint()
        key_list = attn_outputs[0]
        value_list = attn_outputs[1]
        num_example_attn = attn_outputs[2]
        attn_output = attn_outputs[3][0]  # output_attn: a, present, (attentions)
        outputs = attn_outputs[3][1:]
        # residual connection
        ...
        feed_forward_hidden_states, example_list, num_example = self.mlp(hidden_states) #FIXME: example, list
        #breakpoint()
        # residual connection
        hidden_states = residual + feed_forward_hidden_states

        if use_cache:
            outputs = (hidden_states,example_list, num_example, key_list, value_list, num_example_attn) + outputs
        else:
            outputs = (hidden_states,example_list,num_example, key_list, value_list, num_example_attn) + outputs[1:]
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if use_cache is True: 
                presents = presents + (outputs[6],) #FIXME:kvcache: outputs[1] is present(key, value) since use_cache = True
                #breakpoint()

            if output_attentions:
                all_self_attentions = all_self_attentions + (outputs[7 if use_cache else 6],) #FIXME
                if self.config.add_cross_attention:
                    all_cross_attentions = all_cross_attentions + (outputs[8 if use_cache else 7],)#FIXME
        ...
        return BaseModelOutputWithPastAndCrossAttentions(
            ...
            past_key_values=presents, #kvcache: past_key_values is returned as kv_cache
